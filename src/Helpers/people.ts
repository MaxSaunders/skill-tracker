...
export const useGetPeople = () =>
    useQuery<Person[], AxiosError<ApiError>>({
        queryKey: ["people"],
        queryFn: async () => {
...
export const useGetPerson = (id: string, enabled: boolean = true) =>
    useQuery<Person, AxiosError<ApiError>>({
        queryKey: ["person:" + id],
        queryFn: async () => {
...
export const useUpdatePerson = (id: string) => {
    ...
    const post = useCallback(
        async (person: Person) => {
            ...
            setIsLoading(true)
            return axios
                .post(API_URL + "/people/" + id + "/update", { id, name, email, phone })
                .then(() => {
                    setIsLoading(false)
                })
                .catch((err) => {
...
export const useRegisterPerson = (auth0Id?: string, name?: string) =>
    useQuery<Person, AxiosError<ApiError>>({
        ...
        queryFn: async () => {
            ...
            const response = await axios.post(API_URL + "/people/" + auth0Id, {
                auth0: auth0Id,
                name,
            })
            return response.data
