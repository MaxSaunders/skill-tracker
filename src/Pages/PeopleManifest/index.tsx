    ...
    return (
        <>
            ...
            <div className='mb-10'>
                        ...
                        {pageResults.map(({ id, name, skills, topSkill }, peopleIndex) =>
                            <TableRow key={id + ':' + peopleIndex} className='hover:bg-gray-700 h-14'>
                                ...
                                <TableCell className='p-0 w-max min-w-min'>
                                    <Link to={`/people/${id}`} className='p-4 flex sm:grid grid-cols-2 items-center justify-end'>
                                        <span className='font-bold mr-1 text-yellow-500 truncate'>
                                            {topSkill?.name}
                                        </span>
                                        {topSkill?.rating && (
                ...
                <Pager current={page} size={5} setPage={setPage} totalPages={Math.ceil(filteredResults.length / pageSize)} resultsCount={filteredResults.length} />
            </div>
        </>
    )
}
